...
CSV_PATH         = "data/export_cda_normalized.csv"  # CSV 파일 경로
...
# ══════════════════════════════════════════
# STEP 0. 데이터 로드
# long-format CSV → wide-format DataFrame
# ══════════════════════════════════════════

def load_data(path: str) -> pd.DataFrame:
    """
    export_cda_normalized.csv 읽어서
    timestamp | bpm | spo2 | resp_rate 형태로 변환
    """
    df_raw = pd.read_csv(path)

    # timestamp: timezone 제거 후 datetime 변환
    df_raw["timestamp"] = pd.to_datetime(
        df_raw["timestamp"], utc=True
    ).dt.tz_convert("Asia/Seoul").dt.tz_localize(None)

    # ── 심박수 ──────────────────────────────
    hr = df_raw[df_raw["display_name"] == "Heart rate"].copy()
    hr = hr.set_index("timestamp")["value"]
    hr = hr[(hr >= 30) & (hr <= 220)]           # 유효 범위 필터
    hr = hr.resample("1min").mean()
    hr = hr.interpolate(method="time")

    # ── 혈중산소 (SpO2) ──────────────────────
    spo2 = df_raw[df_raw["display_name"] == "Oxygen saturation"].copy()
    ...
    # ── 호흡수 ──────────────────────────────
    resp = df_raw[df_raw["display_name"] == "Respiratory rate"].copy()
...
def print_results(events: list[dict]):
    print(f"\n{'='*50}")
